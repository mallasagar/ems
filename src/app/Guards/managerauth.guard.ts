...
export class  managerauthGuard implements CanActivate {
 ...
 this.userstatusservice.userStatus$.subscribe((data:any)=>{
  if(data){
    this.ismanagerrole=data
  }
})
this.status=Boolean(localStorage.getItem('isloggedin')&&(localStorage.getItem('userrole')==='manager'))
if (this.ismanagerrole==="manager"||this.status) {
  return true;
} else {
